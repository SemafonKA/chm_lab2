...
void MatrixGen(vector<vector<double>>& matrix, size_t zeroDiags) {
   auto rd = mt19937(0);      // зафиксируем рандом, чтобы в дальнейшем удобнее было помен€ть знаки
...
void MatrixOut(vector<vector<double>>& matrix, size_t zeroDiags, ostream&& out) {
   // ¬ыводим диагонали с верхней вниз
   out.precision(15);
...
int main(int argc, char** argv) {
   ...
   {
      cout << "¬ведите размер генерируемой матрицы: ";
      cin >> matrixSize;
      cout << "¬ведите число нулевых диагоналей: ";
      cin >> zeroDiags;
   }

   if (zeroDiags > matrixSize - 5)
   {
      cout << "Ќеверно указано число нулевых диагоналей." << endl;
